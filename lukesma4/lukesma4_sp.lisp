...
(defconstant lukesma4-agent-name "LM")

(defstructure (lukesma4-body 
    (:include db-agent-body(name lukesma4-agent-name)))
)

(defstructure
   (lukesma4
    (:include db-agent
         (program 'lukesma4-program)
         (body (make-lukesma4-body))
         (name lukesma4-agent-name))) 
  "lukesma4 agent for db-world.")


(defun lukesma4-program (percept)
;  "throw ball near enemy agent, then go for it, grab it, throw it at agent, kick it from his hands and do it again"
  (let* (
         (me (car percept))
         (grid (cadr percept))
         (my-loc (find-lukesma4-location grid))
         (my-x (car my-loc))
         (my-y (cadr my-loc))
         ;(agents (db-world-agents grid))
         (ball-loc (find-ball-location grid))
         (ball-x (car ball-loc))
         (ball-y (cadr ball-loc))       
         ...
         (enemy-loc (find-enemy-agent-location grid))
         (enemy-x (car enemy-loc))
         (enemy-y (cadr enemy-loc))
         (target-loc (get-target enemy-x enemy-y my-x my-y))
         ;(target-x (car target))
         ;(target-y (cadr target))
      )
    (cond
        ;((not holding-ball ))
        (( and holding-ball (is-next-to-me enemy-x enemy-y my-x my-y)) `(throw-ball ,@enemy-loc))
        (holding-ball `(throw-ball ,@target-loc))
        (ball-on-my-loc 'grab-ball )
        ((and (not ball-on-my-loc) ( < ball-x my-x)) 'go-left)
        ((and (not ball-on-my-loc) ( > ball-x my-x)) 'go-right)
        ((and (not ball-on-my-loc) ( > ball-y my-y)) 'go-up)
        ((and (not ball-on-my-loc) ( < ball-y my-y)) 'go-down)
        ( t 'stay)
      )  ; in the case when everyone else died
    )
)

(defmethod is-next-to-me(enemy-x enemy-y my-x my-y)
  (cond
   ((and (= enemy-x my-x) (= (- enemy-y 1) my-y)) t)
   ((and (= enemy-x my-x) (= (+ enemy-y 1) my-y)) t)
   ((and (= (- enemy-x 1) my-x) (= enemy-y my-y)) t)
   ((and (= (+ enemy-x 1) my-x) (= enemy-y my-y)) t)
   (t nil)
  ) 
)

(defmethod get-target(enemy-x enemy-y my-x my-y)
  (cond
      ((< my-x enemy-x) (list (- enemy-x 1) enemy-y))
      ((> my-x enemy-x) (list (+ enemy-x 1) enemy-y))
      ((< my-y enemy-y) (list enemy-x (- enemy-y 1))) 
      ((> my-y enemy-y) (list enemy-x (+ enemy-y 1))) 
   )
)
    
(defmethod lukesma4-predicate ((obj percept-object))
  (if (equal (percept-object-name obj) lukesma4-agent-name)
      obj 
      nil
  )
)

(defun find-lukesma4-location (grid)
  (find-X-location #'lukesma4-predicate grid)
)

;(defun can-move-to ((grid x y))
;  (let* ((obj (objects-at ))
;         )
;    )
;)

(defmethod enemy-agent-predicate ((obj percept-object))
...
(defun find-enemy-agent-location (grid)
  (find-X-location #'enemy-agent-predicate grid)
)
