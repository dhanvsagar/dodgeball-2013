...
(defstructure (steklmar-body
	(:include db-agent-body (name "STE"))
)
...
(defun steklmar-program (percept)
	(let* (
		(me (car percept))
		(grid (cadr percept))
		(enemy-loc (steklmar-find-enemy-location grid))
		(ball-on-my-loc
			(member-if
		...
		(loop while (not (null not-visited)) do
			...
			(loop for n-loc in neighbors do
				(cond ((and
					...
					(null (steklmar-enemy-at? n-loc grid))
				)
...
(defun get-closest-enemy (my-loc grid)
	(get-closest-loc my-loc (steklmar-find-all-enemies grid))
)

(defun steklmar-find-all-enemies (grid)
	(let* (
		(enemies nil)
	)
		(dotimes (numberx (car (array-dimensions grid)))
			(dotimes (numbery (cadr (array-dimensions grid)))
				(setf enemy (identify-in-list #'steklmar-enemy-p (aref grid numberx numbery)))
				(if (not (null enemy))
...
(defun steklmar-enemy-at? (location grid)
	(identify-in-list #'steklmar-enemy-p (aref grid (car location) (cadr location)))
)

(defun steklmar-find-enemy-location (grid)
	(find-X-location #'steklmar-enemy-p grid)
)

(defun steklmar-enemy-p ((obj percept-object))
	(if	(and
		(not (equal (percept-object-name obj) "#"))
		(not (equal (percept-object-name obj) "STE"))
