...
;
(defconstant stiplsta "S")

(defstructure (stiplsta    ; replace "my-agent" by your unique name, as e.g. FIT username
                (:include db-agent 
                  (body (make-stiplsta-agent-body))
                  (program 'stiplsta-agent-program)
                  (name stiplsta)
                  ) 
                  )
  )
;
(defstructure (stiplsta-agent-body 
                (:include db-agent-body (name stiplsta)))
...
;
(defun stiplsta-agent-program (percept)
  (let ((agent-body (first percept)))    ; extracts agent body from percept
    (setf percept (second percept))      ; extracts proper percept

(setf me (stiplsta-find-me percept))
(setf him (stiplsta-find-him me (stiplsta-souperi percept)))
(setf ball (stiplsta-find-ball percept))

;(read-char)

;mam mic
;hodim jej pobliz agenta, abych jej mohl vybit a nasledne ziskat mic zpet
;pokud je agent soused, hazim na nej
(if (object-contents agent-body)
  (progn

    (if (identify-in-list #'stiplsta-souper (aref percept (+ (car me) 1) (cadr me))) (return-from stiplsta-agent-program `(throw-ball, (+ (car me) 1), (cadr me)) ) nil)
    (if (identify-in-list #'stiplsta-souper (aref percept (- (car me) 1) (cadr me))) (return-from stiplsta-agent-program `(throw-ball, (- (car me) 1), (cadr me)) ) nil)

    (if (identify-in-list #'stiplsta-souper (aref percept (car me) (+ (cadr me) 1))) (return-from stiplsta-agent-program `(throw-ball, (car me), (+ (cadr me) 1)) ) nil)
    (if (identify-in-list #'stiplsta-souper (aref percept (car me) (- (cadr me) 1))) (return-from stiplsta-agent-program `(throw-ball, (car me), (- (cadr me) 1)) ) nil)

    (if (and (stiplsta-test-field percept (+ (car him) 1) (cadr him)) (< (car him) (car me))) (return-from stiplsta-agent-program `(throw-ball, (+ (car him) 1), (cadr him))) )
    (if (and (stiplsta-test-field percept (- (car him) 1) (cadr him)) (> (car him) (car me))) (return-from stiplsta-agent-program `(throw-ball, (- (car him) 1), (cadr him))) )
    (if (and (stiplsta-test-field percept (car him) (+ (cadr him) 1)) (< (cadr him) (cadr me))) (return-from stiplsta-agent-program `(throw-ball, (car him), (+ (cadr him) 1))) )
    (if (and (stiplsta-test-field percept (car him) (- (cadr him) 1)) (> (cadr him) (cadr me))) (return-from stiplsta-agent-program `(throw-ball, (car him), (- (cadr him) 1))) )
    ) nil
)

;jsem na mici - v kazdem pripade seberu mic
(if (equal me ball) 
    (return-from stiplsta-agent-program 'grab-ball)
 nil
)

;jiny agent je na mici - vyrazim mu mic
;predpokladam ze je to soused
(if (stiplsta-agent-on-ball percept) 
(progn
  (if (< (car me) (car ball)) (return-from stiplsta-agent-program 'go-right) nil)
  (if (> (car me) (car ball)) (return-from stiplsta-agent-program 'go-left) nil)
  (if (< (cadr me) (cadr ball)) (return-from stiplsta-agent-program 'go-up) nil)
  (if (> (cadr me) (cadr ball)) (return-from stiplsta-agent-program 'go-down) nil)
) nil)

;tohle je prusvih - nektery agent drzi mic
;za normalnich okolnosti by k tomu v modu1 nemelo dojit, proto budu postupovat nahodne
(if (null ball)
(return-from stiplsta-agent-program (nth (random 4) '(go-left go-right go-up go-down))) 
nil )

;priprava pro vypocet vzdalenosti od mice
(setf vzdalenosti (make-array (array-dimensions percept)  :initial-element nil) )
 (dotimes (x (car (array-dimensions percept)))
        (dotimes (y (cadr (array-dimensions percept)))
        
        (progn
	      (setf (aref vzdalenosti x y) 999) 
	  (dolist (item (aref percept x y))
(progn
	    (if (or (percept-object-wall-p item) (and (percept-object-agent-p item) (not (equal (percept-object-name item) stiplsta)) ) ) 
	      (setf (aref vzdalenosti x y) nil) 
	      nil
	      )

	    (if (percept-object-ball-p item) 
	      (setf (aref vzdalenosti x y) 0) nil
	      )
 )
)
)))
...
(setf vzdalenosti (stiplsta-zoom vzdalenosti me))


;; (read-char)

;jdu na policko, ktere je blize mici, nez moje soucasna pozice
(if (and (> (car me) 0) (not (null (aref vzdalenosti (- (car me) 1) (cadr me))))  (< (aref vzdalenosti (- (car me) 1) (cadr me)) (aref vzdalenosti (car me) (cadr me))) ) (return-from stiplsta-agent-program 'go-left) nil)
(if (and (< (car me) (- (car (array-dimensions vzdalenosti)) 1)) (not (null (aref vzdalenosti (+ (car me) 1) (cadr me))))   (< (aref vzdalenosti (+ (car me) 1) (cadr me)) (aref vzdalenosti (car me) (cadr me))) ) (return-from stiplsta-agent-program 'go-right) nil)

(if (and (> (cadr me) 0) (not (null (aref vzdalenosti (car me) (- (cadr me) 1))))   (< (aref vzdalenosti (car me) (- (cadr me) 1)) (aref vzdalenosti (car me) (cadr me) )) ) (return-from stiplsta-agent-program 'go-down) nil)
(if (and (< (cadr me) (- (cadr (array-dimensions vzdalenosti)) 1)) (not (null (aref vzdalenosti (car me) (+ (cadr me) 1))))  (< (aref vzdalenosti (car me) (+ (cadr me) 1)) (aref vzdalenosti (car me) (cadr me) )) ) (return-from stiplsta-agent-program 'go-up) nil)
 ...
 ;tahle funkce pocita vzdalenost policka od mice
 ;vyuziva rekurzi
 (defun stiplsta-zoom(vzdalenosti me) 
	(setf konec T)

 (dotimes (x (car (array-dimensions vzdalenosti)))
  
        (dotimes (y (cadr (array-dimensions vzdalenosti)))
          (progn
          
	    (if (and (> x 0) (not (null (aref vzdalenosti (- x 1) y)))  (not (null (aref vzdalenosti x y) ))  (< (aref vzdalenosti (- x 1) y) (aref vzdalenosti x y)) ) (setf (aref vzdalenosti x y) (+ (aref vzdalenosti (- x 1) y) 1) ) nil)
	    (if (and (< x (- (car (array-dimensions vzdalenosti)) 1)) (not (null (aref vzdalenosti (+ x 1) y)))  (not (null (aref vzdalenosti x y) ))  (< (aref vzdalenosti (+ x 1) y) (aref vzdalenosti x y)) ) (setf (aref vzdalenosti x y) (+ (aref vzdalenosti (+ x 1) y) 1) ) nil)

	    (if (and (> y 0) (not (null (aref vzdalenosti x (- y 1))))  (not (null (aref vzdalenosti x y) ))  (< (aref vzdalenosti x (- y 1)) (aref vzdalenosti x y)) ) (setf (aref vzdalenosti x y) (+ (aref vzdalenosti x (- y 1)) 1) ) nil)
	    (if (and (< y (- (cadr (array-dimensions vzdalenosti)) 1)) (not (null (aref vzdalenosti x (+ y 1))))  (not (null (aref vzdalenosti x y) ))  (< (aref vzdalenosti x (+ y 1)) (aref vzdalenosti x y)) ) (setf (aref vzdalenosti x y) (+ (aref vzdalenosti x (+ y 1 )) 1) ) nil)

	    (if (equal (aref vzdalenosti (car me) (cadr me) ) 999) (setf konec nil) nil)
	    
          )))

        (if (null konec) (return-from stiplsta-zoom (stiplsta-zoom vzdalenosti me)) (return-from stiplsta-zoom vzdalenosti) )
 )
 
 
  (defun stiplsta-find-me(grid)
      (dotimes (x (car (array-dimensions grid)))
        (dotimes (y (cadr (array-dimensions grid)))
	  (dolist (item (aref grid x y))

	    (when (and (percept-object-agent-p item) ( equalp (percept-object-name item) stiplsta))

	      (return-from stiplsta-find-me (list x y)) nil)
 ))))
 
  (defun stiplsta-test-field(grid x y) 
    (setf item (aref grid x y))

    (if (equal item nil) (return-from stiplsta-test-field T) nil)

    (if (percept-object-ball-p (car item)) (return-from stiplsta-test-field T) nil)

  )
 
   (defun stiplsta-find-him(me souperi)
    (let ((minimum 10000) (nejblizsi nil) (vzdalenost nil))
        (dolist (souper souperi)
            (setf vzdalenost 
            
            (+
            (expt (abs (- (car me) (car souper))) 2)
            (expt (abs (- (cadr me) (cadr souper))) 2)        ) 
            
            )

                (if (< vzdalenost minimum)
                    (progn
                        (setf minimum vzdalenost)
                        (setf nejblizsi (list (car souper) (cadr souper)) )
                    ) nil
                )
            
        )
        (return-from stiplsta-find-him nejblizsi) 
    )
 )
 
   (defun stiplsta-find-ball(grid)
      (dotimes (x (car (array-dimensions grid)))
        (dotimes (y (cadr (array-dimensions grid)))
	  (dolist (item (aref grid x y))

	    (when (and (percept-object-ball-p item) )

	      (return-from stiplsta-find-ball (list x y)) nil)
 ))))
 
 (defun stiplsta-agent-on-ball (grid)
    (dotimes (x (car (array-dimensions grid)))
        (dotimes (y (cadr (array-dimensions grid)))
            (when (or (and (identify-in-list #'stiplsta-souper (aref grid x y)) (identify-in-list #'stiplsta-mic (aref grid x y)))  )
                (return-from stiplsta-agent-on-ball T)
            )
        )
    )
    nil
 ...
 (defmethod stiplsta-mic ((obj percept-object))
   (if (percept-object-ball-p obj) obj nil)
   )
   
 (defmethod stiplsta-souper ((obj percept-object))
    (if (and
            (percept-object-agent-p obj)
            (not (equal (percept-object-name obj) stiplsta))
        )
        obj
        nil
    )
)
 (defmethod stiplsta-souper-s-micem ((obj percept-object))
    (if (and
            (percept-object-agent-p obj)
            (not (equal (percept-object-name obj) stiplsta))
...
(defun stiplsta-souperi (percept)
    (let ((mista nil))
        (dotimes (x (car (array-dimensions percept)))
            (dotimes (y (cadr (array-dimensions percept)))
                (when (identify-in-list #'stiplsta-souper (aref percept x y))
                    (setf mista (cons (list x y) mista))
                )
            )
        )
        (return-from stiplsta-souperi mista)
    )
)
