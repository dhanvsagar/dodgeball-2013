...
(defconstant hanny-agent-fixpoint-detection 10)

(defstructure (strnaj11   ; replace "my-agent" by your unique name, as e.g. FIT username
                (:include db-agent 
    ...
    (cond (me-holding-ball 
          ...
          ((equal my-location ball-location) 
            (let* ((enemies (hanny-agent-find-enemies percept))
                   (enemies-distances
                     (mapcar
                       (lambda (x) (hanny-agent-manhattan-distance x my-location))
                       enemies
                     )
                   )
                   (neighbors (hanny-agent-neighbors-locations-reachable my-location percept))
                  )
              (cond ((notany (lambda (x) (<= x 2)) enemies-distances)
                     'grab-ball
                    )
                    (T 
                      (setf (hanny-agent-body-turns-since-last-ball-change agent-body) 0)
                      (hanny-agent-move-action my-location (first (hanny-agent-loc-empty neighbors percept)))
                    )
              )
            )
          )
...
(defun hanny-agent-program-decide-what-with-ball (agent my-location percept)
  (let* ((enemies (hanny-agent-find-enemies percept))
         ...
         (defensive-improving-location (hanny-agent-defensive-improving    agent
                                                                           percept
                                                                           my-location
                                                                           (mapcar #'first direct-hits)
                                          )
    ...
    (cond (defensible-hits `(throw-ball ,@(caar defensible-hits)))
          ...
          (T `(throw-ball ,@(first (sort (hanny-agent-neighbors-locations-reachable my-location percept)
                                         (lambda (x y) (< (hanny-agent-manhattan-distance-to-center x)
                                                          (hanny-agent-manhattan-distance-to-center y)
                                                       )
                                         )
                                   )
                            )
...
(defun hanny-agent-defensive-improving (agent grid my-location enemies)
  (let* ((shortest-paths-lens
          (mapcar
            (lambda (x) (hanny-agent-manhattan-distance my-location x))
            enemies
...
(defun hanny-agent-throw-distance (a b)
  (cadar (last (go-through-dist-list a b)))
)
