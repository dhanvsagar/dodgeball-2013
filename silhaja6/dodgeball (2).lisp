...
(defconstant silhaja6-name "silhaja6")
...
(defmethod my_agent-p ((obj percept-object))
  (if (and (not (equal (percept-object-name obj) "#")) 
           (not (equal (percept-object-name obj) silhaja6-name)) 
           (not (equal (percept-object-name obj) "B")))
      obj nil))

(defmethod my_my-loc-p ((obj percept-object))
...
(defun test-agent-mode-1 (student-agent &rest other-agents)
  ...
  (let* ((world (make-db-world :max-steps 170)) (SHD *SURE-HIT-DIST*))
