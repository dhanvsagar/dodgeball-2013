...
(defconstant skvarjak-agent-name "JS")

(defstructure (skvarjak
  (:include db-agent
    (body (make-skvarjak-body))
    (program 'skvarjak-first-step)
    (name skvarjak-agent-name)))
  "Your agent for db-world.")
...
(defun skvarjak-first-step (percept)
  "get ball and throw it"
  (let* ((me (car percept))
    (grid (cadr percept))
    (ball-on-my-loc (member-if (lambda (a) (typep a 'percept-object-ball)) (apply #'aref grid (object-loc me))))
    (ball-loc (find-ball-location grid))
    (holding-ball (object-contents me))
    (my-loc (skvarjak-find-my-location grid))
    (agents-loc (skvarjak-find-other-agents grid))
    (nearest-agent (skvarjak-find-nearest-agent my-loc grid)))

  (cond
    (ball-on-my-loc
      'grab-ball)
    (holding-ball
      (skvarjak-throw-ball my-loc nearest-agent) )
    ((not agents-loc)
      'stop)
    (t (skvarjak-move my-loc (skvarjak-next-move grid my-loc ball-loc))))))

(defun skvarjak-throw-ball (me agent)
  "evaluates if to throw at or next to agent"
  (sleep 1)
  (cond
    ((= (cdr agent) 1)
      (return-from skvarjak-throw-ball `(throw-ball ,@(car agent))) )
    (t
      (return-from skvarjak-throw-ball (skvarjak-throw-next-to-agent me (car agent))))))

(defun skvarjak-throw-next-to-agent (me agent)
  "throw ball next to agent"
  `(throw-ball ,@(skvarjak-next-to-agent me agent)))

(defun skvarjak-next-to-agent (me agent)
  "is standing next to agent"
  (let*((my-x (car me))
    (my-y (cadr me))
    (agent-x (car agent))
    (agent-y (cadr agent)))
...
(defun skvarjak-move (my-loc next-step)
  "make move"
  (sleep 1)
  (cond
     ( (> (car my-loc) (car next-step)) (return-from skvarjak-move 'go-left) )
     ( (< (car my-loc) (car next-step)) (return-from skvarjak-move 'go-right) )
     ( (< (cadr my-loc) (cadr next-step)) (return-from skvarjak-move 'go-up) )
     ( (> (cadr my-loc) (cadr next-step)) (return-from skvarjak-move 'go-down) )
     (t 'stay)
  )
)

(defun skvarjak-next-move (grid start end)
  "calculate next move using BFS"
  (let ((open (list start)) (closed) (path (make-array (array-dimensions grid))) )
    (loop
      (when (null open) (return-from skvarjak-next-move (skvarjak-ret-path path start end)))
      (let* ((node (car open)) (neighbours (skvarjak-neighbours grid node)))
        (when (equal node end) (return-from skvarjak-next-move (skvarjak-ret-path path start end)))
        (setf open (cdr open))
...
(defun skvarjak-ret-path (path start end)
  "return path from BFS"
  (let ((node end) (next (aref path (car end) (cadr end))))
    (loop
      (when (equal next start) (return-from skvarjak-ret-path node))
      (when (null next) (return-from skvarjak-ret-path nil))
      (setf node next)
      (setf next (aref path (car node) (cadr node))))))

(defun skvarjak-neighbours (grid position)
  "safe neighbours steps"
  ...
  (let ((all-neighbours nil)
    ...
    (when (and (< p-x maxx) (skvarjak-ok-move grid (list (+ p-x 1) p-y)))
      (setf all-neighbours (append all-neighbours (list (list (+ p-x 1) p-y)))))
    (when (and (> p-x 0) (skvarjak-ok-move grid (list (- p-x 1) p-y)))
      (setf all-neighbours (append all-neighbours (list (list (- p-x 1) p-y)))))
    (when (and (< p-y maxy) (skvarjak-ok-move grid (list p-x (+ p-y 1))))
      (setf all-neighbours (append all-neighbours (list (list p-x (+ p-y 1))))))
    (when (and (> p-y 0) (skvarjak-ok-move grid (list p-x (- p-y 1))))
      (setf all-neighbours (append all-neighbours (list (list p-x (- p-y 1))))))
    (return-from skvarjak-neighbours all-neighbours)))

(defun skvarjak-ok-move (grid position)
  "is move safe"
  (setf mex (car position))
  (setf mey (cadr position))
  (or (null (aref grid mex mey))
    (identify-in-list #'skvarjak-ball-p (aref grid mex mey))))

(defun skvarjak-find-nearest-agent (my-loc grid)
  "find position of nearest agent"
  (let* ((agents (skvarjak-find-agents-locations grid)))
    (skvarjak-get-nearest-agent-loc my-loc agents nil)))

(defun skvarjak-find-agents-locations (grid)
  "fing locations of agents"
  (let (agents)
    (dotimes (numberx (car (array-dimensions grid)))
      (dotimes (numbery (cadr (array-dimensions grid)))
        (dolist (item (aref grid numberx numbery))
          (when (and (percept-object-agent-p (eval item))
            (not (equal (percept-object-name item) skvarjak-agent-name)))
              (setf agents (push (@ numberx numbery) agents))))))
  (return-from skvarjak-find-agents-locations agents)))

(defun skvarjak-get-nearest-agent-loc (my-loc agents nearest-agent)
  "compute position of nearest agent"
  (cond
    ((null agents)
      nearest-agent)
    ((null nearest-agent)
      (skvarjak-get-nearest-agent-loc my-loc (cdr agents)
        (cons (car agents) (skvarjak-distance my-loc (car agents)))))
    (t
      (let* ((nearest-distance (cdr nearest-agent))
        (next-distance (skvarjak-distance my-loc (car agents))))

        (cond
          ((< next-distance nearest-distance)
            (skvarjak-get-nearest-agent-loc my-loc (cdr agents) (cons (car agents) next-distance)))
          (t
            (skvarjak-get-nearest-agent-loc my-loc (cdr agents) nearest-agent)))))))

(defun skvarjak-distance (l1 l2)
  "manhatann distance"
  (+ (abs (- (car l1) (car l2)))
    (abs (- (cadr l1) (cadr l2)))))

(defun skvarjak-find-other-agents (grid)
  "find all agents but me"
  (let (agents)
    (dotimes (numberx (car (array-dimensions grid)))
      (dotimes (numbery (cadr (array-dimensions grid)))
        (when (identify-in-list #'skvarjak-other-agent-p (aref grid numberx numbery))
          (setf agents (append agents (list(list numberx numbery)))))))
    nil agents))

(defun skvarjak-find-my-location (grid)
  "find my agent location"
  (dotimes (numberx (car (array-dimensions grid)))
    (dotimes (numbery (cadr (array-dimensions grid)))
      (when (identify-in-list #'skvarjak-my-agent-p (aref grid numberx numbery))
        (return-from skvarjak-find-my-location (list numberx numbery)))))
  nil)

(defmethod skvarjak-ball-p ((obj percept-object))
  "my agent predicate"
  (if (equal (percept-object-name obj) "B")
    obj nil))

(defmethod skvarjak-my-agent-p ((obj percept-object))
  "my agent predicate"
  (if (equal (percept-object-name obj) skvarjak-agent-name)
    obj nil))

(defmethod skvarjak-other-agent-p ((obj percept-object))
  "other agents predicate"
  (if (and (not (equal (percept-object-name obj) "#"))
           (not (equal (percept-object-name obj) skvarjak-agent-name))
           (not (equal (percept-object-name obj) "B")))
    obj nil))
